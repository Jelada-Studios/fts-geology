...
public final class VolcanoSummit {
    ...
    static void connectVentDown(ServerLevel level, BlockPos start, int coreX, int coreZ, int floorY) {
        ...
        for (int guard = 0; guard < 400; guard++) {
            if (p.getY() < start.getY() - VEIN_DEPTH) { end = "at its depth"; break; }
            if (level.getBlockState(p).is(Blocks.BEDROCK)) { end = "on bedrock"; break; }
            if (level.getBlockState(p).getFluidState().is(FluidTags.LAVA)) { end = "in lava"; break; }
            ...
            if (localGround == Integer.MIN_VALUE) { end = "over open air"; break; }
            if (p.getY() > localGround - LAVA_SURFACE_CLEARANCE) {
                p = p.below();
                if (p.getY() <= floorY - 3) { end = "at the floor"; break; }
                continue;
            }
        ...
        GeysersMod.LOGGER.debug("Flank vein from {} {} {}: {} cells, ended {} at Y {}",
                start.getX(), start.getY(), start.getZ(), cells, end, p.getY());
    }

    /** No vein goes deeper than this below its outlet: the far ones would otherwise cross every cave on the way. */
    static final int VEIN_DEPTH = 48;

    /** Is this cell, or one of its four horizontal neighbours, open air or water? */
    static boolean openAround(ServerLevel level, BlockPos p) {
        if (isOpen(level.getBlockState(p))) return true;
        for (net.minecraft.core.Direction d : net.minecraft.core.Direction.Plane.HORIZONTAL) {
            if (isOpen(level.getBlockState(p.relative(d)))) return true;
        }
        return false;
    }

    private static boolean isOpen(BlockState s) {
        return s.isAir() || (!s.getFluidState().isEmpty() && !s.getFluidState().is(FluidTags.LAVA));
    }
    ...
    static void placeField(ServerLevel level, Ctx c) {
        ...
        // A big mountain's springs cluster at its foot (Beppu, Hakone), hundreds of blocks from the summit and
        // outside this job's loaded ring: the chunk pass places them as the ground there loads, in the groups
        // VolcanoField.footCluster sets out. Round a small cone the rock is the cone, and springs keep off it.
        if (!isle && c.size == VolcanoSize.LARGE && !ring) return;
